...
```{r clean-up & features, echo = T, results = 'hide'}
...
whole_dataset_pie <- original_data %>%
  ...
  theme(
    ...
    plot.title = element_text(hjust = 0.5, margin = unit(c(0, 0, 20, 0), "pt"))
  )
...
# Check the structure of the variable_importance_df data frame
str(variable_importance_df)

# Create a pie chart of feature importance if the Importance variable exists
if ("Importance" %in% colnames(variable_importance_df)) {
  pie_chart <- ggplot(variable_importance_df, aes(x = 1, y = Importance, fill = row.names(variable_importance_df))) +
    geom_bar(stat = "identity", width = 1) +
    coord_polar("y", start = 0) +
    theme_void() +
    labs(title = "Feature Importance")
  
  # Convert the pie chart to a standard plot object
  pie_plot <- as.ggplot(pie_chart)
  
  # Display the pie plot
  print(pie_plot)
} else {
  print("The 'Importance' variable is not present in the variable_importance_df data frame.")
}
```
